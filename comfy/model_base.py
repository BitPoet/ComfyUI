...
class Ideogram4(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        ...
        self.memory_usage_factor_conds = ("reference_latents",)
    ...
    def extra_conds(self, **kwargs):
        ...
        if reference_latents is not None:
            ...
            out['reference_latents'] = comfy.conds.CONDList([
                self.process_latent_in(reference_latents[0])
            ])
        return out
    ...
    def extra_conds_shapes(self, **kwargs):
        ...
        reference_latents = kwargs.get("reference_latents", None)
        if reference_latents is not None:
            out['reference_latents'] = [
                1,
                reference_latents[0].shape[1],
                math.prod(reference_latents[0].shape[2:]),
            ]
        return out
